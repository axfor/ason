...
- **`simd/archsimd`：三个架构全都用上了**。这条结论我改过两次，两次都是查错了层次，记下来以免再犯。
  ...
  **三者的硬件门槛完全不同，这是 `archsimd` 最容易踩的地方**：包本身**不做任何运行时回退**，`doc.go` 只写
  ...
  - **amd64 的门槛是 AVX2 一刀切**，而且**"退到 16 字节"根本不是退路**——`Uint8x32.Equal` 与 `Mask8x32.ToBits`
    ...
    **但要说清这个门槛的性质**：查 `compare_gen_amd64.go` 可见 `Less` 展开成"翻符号位 + 有符号比较"三条指令，
    SSE2 早就有；AVX2 这个要求是经 `BroadcastInt8x16`（`SetElem` + `broadcast1To2`）传上来的。
    **是特性标注按最保守依赖链传递的结果，不是指令本身需要 AVX2**。手写 SSE2 层的价值在于绕开这条依赖链。
  ...
  **arm64 这条要纠正一次**：我原先记"Go 汇编器的 arm64 指令集没有 `VCMHI`（无符号大于）也没有 `VUMINV`，
  所以手写只能 `VUSHR $5` 后与零比较、取位置只能用魔数掩码 + 两次 `VMOV` + `RBIT`/`CLZ`"——**这是查错了**。
  查 Go 源码三处都在：`obj/arm64/a.out.go` 有 `AVCMHI`(1147) 与 `AVUMINV`(1136)，`anames.go` 有对应字符串，
  `asm7.go` 里 `oprangeset` 与 `case` 分支都有编码实现。线索本来就在眼前：archsimd 版的反汇编里
  明明打出了 `VCMHI V1.B16, V3.B16, V1.B16` 和 `VUMINV V0.B16, V0`——**能被 `objdump` 反汇编出的助记符，
  汇编器必然认得**。当时我只 grep 了 `anames.go` 就下结论，方式不对。
  **后果是实打实的**：手写 NEON 本可以用同样的指令序列，**默认构建（不开实验）的 arm64 也能拿到那 10~25%**，
  不必等实验开关。这是"用不了才自己写"这条原则下，我误判了"用不了"的一次。
...
8. **已完成 · arm64 向量化字符串体扫描**：Go 不为任何架构自动向量化，所以用汇编写了 NEON 版（16 字节一步，
   ...
   **SSE2 的验证路径与结果**：开发机是 arm64，SSE2 的逻辑在本机只能验证到"能汇编、能交叉编出 amd64 测试二进制、
   分流选择正确"，执行验证靠 CI 的 amd64 job（`ubuntu-latest` 跑 `go test -race ./...`，其中包含逐字节参考实现
   对照与"加速器契约"穷举测试，两者已改为架构中立、两边共用）。**已兑现**：CI run `34690259151` 全部通过，
   所以这两份汇编各自都在自己的架构上真实执行过并与参考实现一致。
