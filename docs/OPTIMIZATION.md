...
2. **区域内的 SWAR 结构扫描**：8 字节一组找出 `" { } [ ] \` 与首个非结构字节，配合 popcount 更新深度；
   字面量校验仍逐字节但只在标量段内。预期结构密集体 1.5–2×。复杂度最高，但 profile 已经指向它：
   密集 tools 整块交付（673 MB/s）的时间是 `scan` 54.6%（累计 77.3%）+ `scanStringBody` 20.5%，
   剩下的 20.5% 是 `runtime.madvise`（运行时归还内存，与引擎无关）。写穿没有改变这个分布。
3. **零拷贝 key**（降级）：key 与其周围空白不跨块时，`kvRaw` 直接切片输入缓冲，跨块才拷贝。
