...
type Writer struct {
	...
	// far into the input it has got. The first byte that is not a straight pass-through materialises the run
	// (one copy of what was skipped so far) and the writer goes back to the normal buffer. A chunk that never
	// materialises costs no copy at all: Out() hands back a slice of the caller's input.
	vp   []byte
	vlen int
	virt bool
}
...
func (w *Writer) materialise() {
	...
	if w.vlen > 0 {
		w.reserve(w.vlen + 64)
		w.buf = append(w.buf, w.vp[:w.vlen]...)
	} else {
		w.take()
	}
}
